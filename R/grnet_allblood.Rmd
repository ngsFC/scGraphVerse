...
est <- list()
tictoc::tic()
for (k in seq_along(count_matrices)) { 
est[[k]] <- PCzinb(as.matrix(count_matrices[[k]]), method="zinb1", maxcard=2)
}
tictoc::toc()
```
...
for (k in seq_along(est)) {
          rownames(est[[k]]) <- rownames(adjm)
          colnames(est[[k]]) <- colnames(adjm)
        }
```

```{r}
consesusm <- create_consensus(est, method="vote")
consesusu <- create_consensus(est, method="union")

scores.pc <- pscores(adjm, list(consesusm))
scoresu.pc <- pscores(adjm, list(consesusu))

scores.pc$Statistics %>%
    datatable(extensions = 'Buttons',
              options = list(
                dom = 'Bfrtip',
                buttons = c('csv', 'excel'),
