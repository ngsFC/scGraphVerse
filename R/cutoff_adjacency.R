...
cutoff_adjacency <- function(count_matrices, weighted_adjm_list, n, method = "GRNBoost2", weight_function = "mean", nCores = NULL) {
        ...
        if (all(sapply(count_matrices, function(x) inherits(x, "Seurat")))) {
          ...
          count_matrices <- lapply(count_matrices, as.matrix) # Convert to matrix format
          count_matrices <- lapply(count_matrices, t) # Convert to matrix format
        }
        
        # Detect if count_matrices is a list of SingleCellExperiment (SCE) objects
        if (all(sapply(count_matrices, function(x) inherits(x, "SingleCellExperiment")))) {
          message("Detected SingleCellExperiment objects. Extracting logcounts matrices...")
          
          count_matrices <- lapply(count_matrices, function(sce) {
