...
#' determined using the 99th percentile of weights from shuffled matrices. The function supports 
#' different network inference methods such as GRNBoost2, GENIE3, and JRF.
#'
...
#' @param method The network inference method to use, one of: "GRNBoost2", "GENIE3", or "JRF".
#' @param weight_function The function used to symmetrize adjacency matrices. Default is "mean".
#' @param nCores The number of cores to use for parallel computation. Default is the number of available cores.
#' 
#' @return A list of binary adjacency matrices (0s and 1s) for each count matrix, obtained by applying
#'   the computed percentile cutoff to the weighted adjacency matrices.
#' 
