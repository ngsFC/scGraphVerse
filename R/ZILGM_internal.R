...
to_upper <- function(tX) {
    tX[lower.tri(tX, diag = FALSE)]
}
...
f_K_fold <- function(Nobs, K = 5) {
    ...
    l <- lapply(seq.int(K), function(x, k, d) {
        list(
            train = d[!(seq(d) %in% seq(k[x, 1], k[x, 2]))],
...
theta_ml2 <- function(y, mu, posz, prob0, bvec, lambda, weights,
    penalty.factor) {
    n <- length(y)
    fit <- optimize(nb_objective,
        y = y, posz = posz, bvec = bvec,
        lambda = lambda, penalty.factor = penalty.factor,
        prob = prob0, mu = mu, weights = weights,
        interval = c(1e-4, 1e+4), maximum = FALSE
    )
    theta <- fit$minimum
    return(theta)
}
...
pNBII <- function(q, mu = 1, sigma = 1, lower.tail = TRUE, log.p = FALSE) {
    ...
    if (length(sigma) > 1) {
        cdf <- ifelse(sigma > 1e-04, pnbinom(q,
            size = mu / sigma,
            mu = mu, lower.tail = lower.tail,
            log.p = log.p
        ),
        ...
        cdf <- if (sigma < 1e-04) {
            ...
            pnbinom(q,
                size = mu / sigma, mu = mu, lower.tail = lower.tail,
                log.p = log.p
            )
        }
    }
...
qNBII <- function(p, mu = 1, sigma = 1, lower.tail = TRUE, log.p = FALSE) {
    ...
    if (length(sigma) > 1) {
        q <- ifelse(sigma > 1e-04, qnbinom(p,
            size = mu / sigma,
            mu = mu, lower.tail = lower.tail,
            log.p = log.p
        ),
