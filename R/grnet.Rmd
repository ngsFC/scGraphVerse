...
adj_matrix_list[[1]] %>%
    datatable(extensions = 'Buttons',
              options = list(
                ...
                scrollX = TRUE,
                pageLength = 10), 
              caption = "GRNBoost2 EarlyJ adjacency")

modify_adjacency <- function(matrix) {
  matrix[matrix > 0.936] <- 1
  diag(matrix) <- 1
  matrix[matrix <= 0.936] <- 0
  return(matrix)
}

adj_matrix_list <- lapply(adj_matrix_list, modify_adjacency)
