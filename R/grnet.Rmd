...
```{r}
library(INetTool)
# gene as row and samples as column
list_genie3_norm <- lapply(genie3_late_wadj, function(mat) mat / max(mat))

Con <- consensusNet(list_genie3_norm, theta=0.04, ncores = 15) 
Con$graphConsensus 
Con$Comparison
Con$similarGraphs

consensus_adj <- as.matrix(as_adjacency_matrix(Con$graphConsensus))
```

```{r}
scoresinet.genie3.late <- pscores(adjm, list(consensus_adj))

scoresinet.genie3.late$Statistics %>%
    datatable(extensions = 'Buttons',
              options = list(
                dom = 'Bfrtip',
                buttons = c('csv', 'excel'),
...
```{r GENIE3 compare late, fig.width=8,fig.width=10}
ajm_compared <- compare_consensus(consensus_adj, adjm)
```
