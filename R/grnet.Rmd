...
plotROC(sjrf_mat, adjm, plot_title = "ROC curve - jrf Late Integration")
...
sjrf_mat_adj <- cutoff_adjacency(count_matrices = count_matrices,
                 weighted_adjm_list = sjrf_mat_wadj, 
                 n = 2,
                 method = "JRF")
...
```{r jrf metrics late}
scores <- pscores(adjm, sjrf_late_adj)
...
```{r jrf plotg late}
plots <- plotg(sjrf_late_adj)
```

```{r jrf compare late, fig.width=8,fig.width=10}
ajm_compared <- compare_consensus(sjrf_late_adj, adjm)
```
