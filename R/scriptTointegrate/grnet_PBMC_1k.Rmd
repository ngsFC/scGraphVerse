...
```{r}
seurat_object <- readRDS("/home/francescoc/Desktop/scGRN_simulation/data/Szabo_PBMC_2021.rds")
...
for (cell_type in cell_types) {
  ...
  top_genes <- names(sort(avg_expression, decreasing = TRUE))[1:5000]
  top_genes_per_cell_type[[cell_type]] <- top_genes
}

mart <- useEnsembl(biomart = "genes", dataset = "hsapiens_gene_ensembl")
ensembl_ids <- top_genes_per_cell_type$`mature NK T cell`[1:1000] # Top 1000 genes
...
top_genes <- gene_mapping$hgnc_symbol
...
for (i in 1:nrow(interactions)) {
  protein1 <- interactions$from[i]
  protein2 <- interactions$to[i]
  score <- interactions$combined_score[i]
  
  adj_matrix[protein1, protein2] <- score
  adj_matrix[protein2, protein1] <- score
}

ensp_ids <- rownames(adj_matrix)  # Replace with your ENSP IDs

mapping <- getBM(
  attributes = c("ensembl_peptide_id", "external_gene_name"),
  filters = "ensembl_peptide_id",
  values = sub("9606\\.", "", ensp_ids),  # Remove species prefix (9606.)
  mart = mart
)
...
remove_disconnected_nodes <- function(adj_matrix) {
  connected_nodes <- rowSums(adj_matrix) > 0 | colSums(adj_matrix) > 0
  filtered_matrix <- adj_matrix[connected_nodes, connected_nodes]
  return(filtered_matrix)
}

adjm <- remove_disconnected_nodes(adjm)
```
...
ggraph(gtruth, layout = "fr") +
  ...
  geom_node_point(aes(size = igraph::degree(gtruth), color = igraph::degree(gtruth))) +
  scale_color_gradient(low = "steelblue", high = "red") +
  theme_void() +
  ggtitle(paste("Ground Truth\nNodes:", num_nodes, "Edges:", num_edges))
