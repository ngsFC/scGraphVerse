...
seurat_url <- "https://datasets.cellxgene.cziscience.com/8e64f5c1-e56c-4b0a-bc83-1447fed2e7a4.rds" # Replace with direct file URL

dest_file <- "./../data/seurat_object.rds" # Or use .qs if it's a qs file
GET(seurat_url, write_disk(dest_file, overwrite = TRUE))

seurat_object <- readRDS(dest_file) 
...
gene_lengths <- getBM(
  ...
  values = rownames(seurat_object[["RNA"]]@data),
  mart = mart
)
...
cell_types <- unique(seurat_object@meta.data$cell_type)
top_genes_per_cell_type <- lapply(cell_types, function(cell_type) {
  cells_in_type <- which(seurat_object@meta.data$cell_type == cell_type)
  if (length(cells_in_type) == 0) return(NULL)
  avg_expression <- rowMeans(tpm_matrix[, cells_in_type, drop = FALSE])
  names(sort(avg_expression, decreasing = TRUE))[1:5000]
})
names(top_genes_per_cell_type) <- cell_types

ensembl_ids <- top_genes_per_cell_type$`mature NK T cell`[1:200]
gene_mapping <- getBM(
...
top_genes <- gene_mapping$hgnc_symbol[gene_mapping$hgnc_symbol != ""]

# Map genes to STRING IDs and retrieve interactions
string_db <- STRINGdb$new(version = "11.5", species = 9606, score_threshold = 900)
...
adj_matrix <- matrix(0, nrow = length(unique_proteins), ncol = length(unique_proteins),
                     dimnames = list(unique_proteins, unique_proteins))
for (i in seq_len(nrow(interactions))) {
  adj_matrix[interactions$from[i], interactions$to[i]] <- interactions$combined_score[i]
  adj_matrix[interactions$to[i], interactions$from[i]] <- interactions$combined_score[i]
}

# Map ENSP IDs to gene names
ensp_ids <- rownames(adj_matrix)
mapping <- getBM(
...
rownames(adj_matrix) <- id_to_gene[sub("9606\\.", "", rownames(adj_matrix))]
colnames(adj_matrix) <- id_to_gene[sub("9606\\.", "", colnames(adj_matrix))]

valid_genes <- !is.na(rownames(adj_matrix)) & !is.na(colnames(adj_matrix))
adj_matrix <- adj_matrix[valid_genes, valid_genes]
adjm <- ifelse(adj_matrix > 0, 1, 0)
adjm <- adjm[rowSums(adjm) > 0 | colSums(adjm) > 0, rowSums(adjm) > 0 | colSums(adjm) > 0]
adjm <- adjm[order(rownames(adjm)), order(colnames(adjm))]
