---
title: "Simulations Inferred network and Performances Comparison"
author: "Francesco Cecere"
date: "`r Sys.Date()`"
output:
  html_document:
    code_folding: hide
    theme: cerulean
    df_print: paged
    toc: yes
    toc_float: yes
  pdf_document:
    toc: yes
---

```{r}
knitr::opts_knit$set(root.dir = "/home/francescoc/Desktop/scGraphVerse/analysis/simulation/results",message=FALSE, warning=FALSE)
```

```{r}
library(dplyr)
library(purrr)
library(patchwork)
library(tidyr)
library(ggplot2)
```


```{r}
met_files <- c("/home/francescoc/Desktop/scGraphVerse/analysis/simulation/results/met_n100_p100.txt", "/home/francescoc/Desktop/scGraphVerse/analysis/simulation/results/met_n100_p200.txt", "/home/francescoc/Desktop/scGraphVerse/analysis/simulation/results/met_n100_p500.txt")

read_and_fulljoin <- function(files) {
  df_list <- lapply(files, read.table, header = TRUE, sep = "\t", stringsAsFactors = FALSE)
  full_joined_df <- Reduce(function(x, y) full_join(x, y), df_list)
  return(full_joined_df)
}

met_data <- read_and_fulljoin(met_files)
```


```{r}
met_late <- met_data %>%
  mutate(Ratio = as.character(Ratio),
         p = as.character(p)) %>%
  filter(Predicted_Matrix %in% c("vote", "union", "inet"))

met_late_long <- met_late %>%
  pivot_longer(cols = c("TPR", "FPR", "Precision", "F1", "MCC", "Time_in_Minutes"), 
               names_to = "Metric", values_to = "Value") %>%
  mutate(Metric = factor(Metric, levels = c("TPR", "FPR", "F1", "Precision", "MCC", "Time_in_Minutes")),
         p = factor(p, levels = c("82", "217", "554")))

plot_base <- ggplot(met_late_long, aes(x = p, y = Value, 
                                       color = Method, 
                                       shape = Predicted_Matrix,
                                       group = interaction(Method, Predicted_Matrix))) +
  geom_line(size = 0.4, alpha=0.7) +  # Slightly increase line size and adjust transparency
  geom_point(size = 2, alpha=0.8) +    # Increase point size for better visibility
  labs(x = "N.Genes", y = "Value", color = "Method", shape = "Late Strategy") +
  theme_minimal() +
  scale_color_manual("Methods", values = c("GENIE3"="darkblue", "GRNBoost2"="forestgreen", "JRF"="purple")) +
  ggtitle("n=100") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "bottom",
        plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))

plot_final <- plot_base + facet_wrap(~Metric, scales = "free", ncol = 3)
print(plot_final)

ggsave("./../analysis/simulation/plot/late_dots_n100.png", plot_final, width = 8, height = 5, dpi = 300, bg = "white")

plot_base <- ggplot(met_late_long, aes(x = p, y = Value, 
                                       color = Method, 
                                       linetype = Predicted_Matrix,  # Change to linetype
                                       group = interaction(Method, Predicted_Matrix))) +
  geom_line(size = 0.7, alpha=0.8) +  # Increase line size for better visibility
  geom_point(size = 1, alpha=0.8) +    # Increase point size for better visibility
  labs(x = "N.Genes", y = "Value", color = "Method", shape = "Late Strategy") +
  theme_minimal() +
  scale_color_manual("Methods", values = c("GENIE3"="darkblue", "GRNBoost2"="forestgreen", "JRF"="purple")) +
  scale_linetype_manual("Late Strategy", values = c("vote"="solid", "union"="dashed", "inet"="dotted")) +
  ggtitle("n=100") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "bottom",
        plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))

plot_final <- plot_base + facet_wrap(~Metric, scales = "free", ncol = 3)

print(plot_final)
ggsave("./../analysis/simulation/plot/late_line_n100.png", plot_final, width = 8, height = 5, dpi = 300, bg = "white")

```

```{r}
met_comp <- met_data %>%
  mutate(Ratio = as.character(Ratio),
         p = as.character(p)) %>%
  filter(!Predicted_Matrix %in% c("inet", "union")) %>%
  mutate(
    Predicted_Matrix = ifelse(Predicted_Matrix == "vote", "late", Predicted_Matrix),
    Predicted_Matrix = ifelse(Method == "JRF", "joint", Predicted_Matrix)  # Replace JRF with joint
  )

met_comp_long <- met_comp %>%
  pivot_longer(cols = c("TPR", "FPR", "Precision", "F1", "MCC", "Time_in_Minutes"), 
               names_to = "Metric", values_to = "Value") %>%
  mutate(Metric = factor(Metric, levels = c("TPR", "FPR", "F1", "Precision", "MCC", "Time_in_Minutes")),
         p = factor(p, levels = c("82", "217", "554")))

plot_base <- ggplot(met_comp_long, aes(x = p, y = Value, 
                                       color = Method, 
                                       linetype = Predicted_Matrix,  # Change to linetype
                                       group = interaction(Method, Predicted_Matrix))) +
  geom_line(size = 0.7, alpha=0.8) +  # Increase line size for better visibility
  geom_point(size = 1, alpha=0.8) +    # Increase point size for better visibility
  labs(x = "N.Genes", y = "Value", color = "Method", shape = "Late Strategy") +
  theme_minimal() +
  scale_color_manual("Methods", values = c("GENIE3"="darkblue", "GRNBoost2"="forestgreen", "JRF"="purple")) +
  scale_linetype_manual("Late Strategy", values = c("late"="solid", "early"="dashed", "joint"="dotted")) +  # Custom line types
  ggtitle("n=100") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "bottom",
        plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))
plot_final <- plot_base + facet_wrap(~Metric, scales = "free", ncol = 3)

print(plot_final)
ggsave("./../analysis/simulation/plot/metrics_comp.png", plot_final, width = 8, height = 5, dpi = 300, bg = "white")

```




```{r}
met_files <- c("/home/francescoc/Desktop/scGraphVerse/analysis/simulation/results/met_n500_p100.txt", "/home/francescoc/Desktop/scGraphVerse/analysis/simulation/results/met_n500_p200.txt", "/home/francescoc/Desktop/scGraphVerse/analysis/simulation/results/met_n500_p500.txt")

read_and_fulljoin <- function(files) {
  df_list <- lapply(files, read.table, header = TRUE, sep = "\t", stringsAsFactors = FALSE)
  full_joined_df <- Reduce(function(x, y) full_join(x, y), df_list)
  return(full_joined_df)
}

met_data <- read_and_fulljoin(met_files)
```


```{r}
met_late <- met_data %>%
  mutate(Ratio = as.character(Ratio),
         p = as.character(p)) %>%
  filter(Predicted_Matrix %in% c("vote", "union", "inet"))

met_late_long <- met_late %>%
  pivot_longer(cols = c("TPR", "FPR", "Precision", "F1", "MCC", "Time_in_Minutes"), 
               names_to = "Metric", values_to = "Value") %>%
  mutate(Metric = factor(Metric, levels = c("TPR", "FPR", "F1", "Precision", "MCC", "Time_in_Minutes")),
         p = factor(p, levels = c("82", "217", "554")))

plot_base <- ggplot(met_late_long, aes(x = p, y = Value, 
                                       color = Method, 
                                       shape = Predicted_Matrix,
                                       group = interaction(Method, Predicted_Matrix))) +
  geom_line(size = 0.4, alpha=0.7) +  # Slightly increase line size and adjust transparency
  geom_point(size = 2, alpha=0.8) +    # Increase point size for better visibility
  labs(x = "N.Genes", y = "Value", color = "Method", shape = "Late Strategy") +
  theme_minimal() +
  scale_color_manual("Methods", values = c("GENIE3"="darkblue", "GRNBoost2"="forestgreen", "JRF"="purple")) +
  ggtitle("n=500") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "bottom",
        plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))

plot_final <- plot_base + facet_wrap(~Metric, scales = "free", ncol = 3)
print(plot_final)

ggsave("./../analysis/simulation/plot/late_dots_n500.png", plot_final, width = 8, height = 5, dpi = 300, bg = "white")

plot_base <- ggplot(met_late_long, aes(x = p, y = Value, 
                                       color = Method, 
                                       linetype = Predicted_Matrix,  # Change to linetype
                                       group = interaction(Method, Predicted_Matrix))) +
  geom_line(size = 0.7, alpha=0.8) +  # Increase line size for better visibility
  geom_point(size = 1, alpha=0.8) +    # Increase point size for better visibility
  labs(x = "N.Genes", y = "Value", color = "Method", shape = "Late Strategy") +
  theme_minimal() +
  scale_color_manual("Methods", values = c("GENIE3"="darkblue", "GRNBoost2"="forestgreen", "JRF"="purple")) +
  scale_linetype_manual("Late Strategy", values = c("vote"="solid", "union"="dashed", "inet"="dotted")) +
  ggtitle("n=500") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "bottom",
        plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))

plot_final <- plot_base + facet_wrap(~Metric, scales = "free", ncol = 3)

print(plot_final)
ggsave("./../analysis/simulation/plot/late_line_n500.png", plot_final, width = 8, height = 5, dpi = 300, bg = "white")

```

```{r}
met_comp <- met_data %>%
  mutate(Ratio = as.character(Ratio),
         p = as.character(p)) %>%
  filter(!Predicted_Matrix %in% c("inet", "union")) %>%
  mutate(
    Predicted_Matrix = ifelse(Predicted_Matrix == "vote", "late", Predicted_Matrix),
    Predicted_Matrix = ifelse(Method == "JRF", "joint", Predicted_Matrix)  # Replace JRF with joint
  )

met_comp_long <- met_comp %>%
  pivot_longer(cols = c("TPR", "FPR", "Precision", "F1", "MCC", "Time_in_Minutes"), 
               names_to = "Metric", values_to = "Value") %>%
  mutate(Metric = factor(Metric, levels = c("TPR", "FPR", "F1", "Precision", "MCC", "Time_in_Minutes")),
         p = factor(p, levels = c("82", "217", "554")))

plot_base <- ggplot(met_comp_long, aes(x = p, y = Value, 
                                       color = Method, 
                                       linetype = Predicted_Matrix,  # Change to linetype
                                       group = interaction(Method, Predicted_Matrix))) +
  geom_line(size = 0.7, alpha=0.8) +  # Increase line size for better visibility
  geom_point(size = 1, alpha=0.8) +    # Increase point size for better visibility
  labs(x = "N.Genes", y = "Value", color = "Method", shape = "Late Strategy") +
  theme_minimal() +
  scale_color_manual("Methods", values = c("GENIE3"="darkblue", "GRNBoost2"="forestgreen", "JRF"="purple")) +
  scale_linetype_manual("Late Strategy", values = c("late"="solid", "early"="dashed", "joint"="dotted")) +  # Custom line types
  ggtitle("n=500") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "bottom",
        plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))
plot_final <- plot_base + facet_wrap(~Metric, scales = "free", ncol = 3)

print(plot_final)
ggsave("./../analysis/simulation/plot/metrics_comp_n500.png", plot_final, width = 8, height = 5, dpi = 300, bg = "white")

```

```{r}
met_files <- c("/home/francescoc/Desktop/scGraphVerse/analysis/simulation/results/met_n100_p500_k1.txt", "/home/francescoc/Desktop/scGraphVerse/analysis/simulation/results/met_n100_p500_k2.txt", "/home/francescoc/Desktop/scGraphVerse/analysis/simulation/results/met_n100_p500_k3.txt",
"/home/francescoc/Desktop/scGraphVerse/analysis/simulation/results/met_n100_p500_k4.txt",
"/home/francescoc/Desktop/scGraphVerse/analysis/simulation/results/met_n100_p500_k5.txt")

met_data <- read_and_fulljoin(met_files)

met_comp <- met_data %>%
  mutate(Ratio = as.character(Ratio),
         p = as.character(p)) %>%
  filter(!Predicted_Matrix %in% c("inet", "union")) %>%
  mutate(
    Predicted_Matrix = ifelse(Predicted_Matrix == "vote", "late", Predicted_Matrix),
    Predicted_Matrix = ifelse(Method == "JRF", "joint", Predicted_Matrix)  # Replace JRF with joint
  )

met_comp_long <- met_comp %>%
  pivot_longer(cols = c("TPR", "FPR", "Precision", "F1", "MCC", "Time_in_Minutes"), 
               names_to = "Metric", values_to = "Value") %>%
  mutate(Metric = factor(Metric, levels = c("TPR", "FPR", "F1", "Precision", "MCC", "Time_in_Minutes")),
         p = factor(p, levels = c("82", "217", "554")),
         k = factor(k, levels = c("1", "2", "3", "4", "5")))

plot_base <- ggplot(met_comp_long, aes(x = k, y = Value, 
                                       color = Method, 
                                       linetype = Predicted_Matrix,  # Change to linetype
                                       group = interaction(Method, Predicted_Matrix))) +
  geom_line(size = 0.7, alpha=0.8) +  # Increase line size for better visibility
  geom_point(size = 1, alpha=0.8) +    # Increase point size for better visibility
  labs(x = "k matrices", y = "Value", color = "Method", shape = "Late Strategy") +
  theme_minimal() +
  scale_color_manual("Methods", values = c("GENIE3"="darkblue", "GRNBoost2"="forestgreen", "JRF"="purple")) +
  scale_linetype_manual("Integration", values = c("late"="solid", "early"="dashed", "joint"="dotted")) +  # Custom line types
  ggtitle("n=100 p=500") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "bottom",
        plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))
plot_final <- plot_base + facet_wrap(~Metric, scales = "free", ncol = 3)

print(plot_final)
ggsave("./../analysis/simulation/plot/metrics_comp_k.png", plot_final, width = 8, height = 5, dpi = 300, bg = "white")

```



