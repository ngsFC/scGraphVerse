...
ddir <- "/home/francescoc/Desktop/scGraphVerse/data/"
pdir <- "/home/francescoc/Desktop/scGraphVerse/data/plots/"
...
```{r}
adjm <- as.matrix(read.table("./../analysis/adjm_top200.txt"))
...
count_matrices <- readRDS("./../analysis/sim_n200p100.RDS")
...
keepscores <- scores.late$Statistics %>% mutate(Predicted_Matrix=c("vote", "union", "inet"), Method=c("GENIE3"), Ratio=nrow(adjm)/ncol(count_matrices[[1]]))
```
...
keepscores <- scores.early$Statistics %>% mutate(Predicted_Matrix=c("early"), Method=c("GENIE3"), Ratio=nrow(adjm)/ncol(count_matrices[[1]])) %>%
...
keepscores <- scores.late$Statistics %>% mutate(Predicted_Matrix=c("vote", "union", "inet"), Method=c("GRNBoost2"), Ratio=nrow(adjm)/ncol(count_matrices[[1]])) %>%
  full_join(keepscores)
...
keepscores <- scores.early$Statistics %>% mutate(Predicted_Matrix=c("early"), Method=c("GRNBoost2"), Ratio=nrow(adjm)/ncol(count_matrices[[1]])) %>%
  full_join(keepscores)
...
keepscores <- scores.jrf.all$Statistics %>% mutate(Predicted_Matrix=c("vote", "union", "inet"), Method=c("JRF"), Ratio=nrow(adjm)/ncol(count_matrices[[1]])) %>%
  full_join(keepscores)
...
```{r}
write.table(keepscores, paste(ddir, "met3.txt", sep = ""), quote = F, col.names = T, row.names = F, sep = "\t")
...
time_data <- time_data %>% mutate(Ratio = nrow(adjm)/ncol(count_matrices[[1]]))
# Splitting the Method column into separate columns
time_data <- time_data %>%
  separate(Method, into = c("Method_Base", "Integration_Step", "Cores"), sep = "_", fill = "right") %>%
  mutate(Integration_Step = ifelse(Method_Base == "JRF", "joint", Integration_Step))

write.table(time_data, paste(ddir, "time3.txt", sep = ""), quote = F, col.names = T, row.names = T)
