...
m1 <- zinb_simdata(ncell, nodes, adjm, mu_range = c(1,4), mu_noise = 1, theta = 1, pi = 0.2, seed = 123)
m2 <- zinb_simdata(ncell, nodes, adjm, mu_range = c(1,7), mu_noise = 3, theta = 0.7, pi = 0.2, seed = 3455)
m3 <- zinb_simdata(ncell, nodes, adjm, mu_range = c(1,10), mu_noise = 5, theta = 0.5, pi = 0.2, seed = 6787)
...
#' @param depth_range Numeric vector of length 2. Range of total counts per cell (e.g., `c(500, 5000)`).
...
#' @importFrom countreg rzinbinom
#' @export
zinb_simdata <- function(n, p, B, mu_range, mu_noise, theta, pi, kmat = 1, depth_range = c(500, 5000), seed = NULL) {
  ...
  stopifnot(is.numeric(depth_range), length(depth_range) == 2, all(depth_range > 0))
  if (!is.null(seed)) stopifnot(is.numeric(seed), length(seed) == 1, floor(seed) == seed)
  ...
  B <- (B + t(B)) / 2  # Ensure symmetry
  
  matrices <- vector("list", kmat)
  
  for (k in seq_len(kmat)) {
    mu <- sample(seq(mu_range[[k]][1], mu_range[[k]][2]), p, replace = TRUE)
    ...
    X <- sweep(X, 1, rowSums(X), FUN = "/")  # Divide by row sum
...
Standard scRNA-seq (10x, Drop-seq, inDrop)

```         
depth_range = c(10,000, 50,000)
Best for gene expression clustering, trajectory inference, and differential expression.
```

2️⃣ High-Depth (Smart-seq2, full-length methods)

```         
depth_range = c(50,000, 500,000)
Ideal for capturing full-length transcripts, isoforms, and high-precision analysis.
```

3️⃣ Low-Depth (Large-scale experiments)

```         
depth_range = c(1,000, 10,000)
Used for large datasets (e.g., single-nucleus RNA-seq, 10x v3 with many cells).
```

4️⃣ Ultra-Low (Pseudobulk, single-nucleus RNA-seq)

```         
depth_range = c(500, 5,000)
Used when focusing on cell types rather than individual cell transcriptomes.
```
