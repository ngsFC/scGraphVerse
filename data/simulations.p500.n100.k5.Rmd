...
df2 <- rbind(df2,df2)
df2$Predicted_Matrix <- c("late","late","late", "early","early","early")
...
for (k in 2:5) {
  ...
  cat("Processing k =", k, "\n")
  count_matrices <- lapply(count_matrices, t)
  count_matrices_k <- count_matrices[1:k]
  ...
  for (method in methods) {
    ...
    keepscores <- consensus_scores$Statistics %>%
      mutate(Predicted_Matrix = c("vote", "union", "inet"), Method = method, Ratio = nrow(adjm)/ncol(count_matrices_k[[1]]), p = nrow(adjm), k = k) %>%
      bind_rows(keepscores)

    ## EARLY INTEGRATION (only for GENIE3 & GRNBoost2)
    if (method %in% c("GENIE3", "GRNBoost2")) {
      ...
      keepscores <- scores_early$Statistics %>%
        mutate(Predicted_Matrix = "early", Method = method, Ratio = nrow(adjm)/ncol(count_matrices_k[[1]]), p = nrow(adjm), k = k) %>%
        bind_rows(keepscores)
    }
  }

  ## Write results for this k
  df_time <- data.frame(
    Method = names(time),
    Time_in_Seconds = sapply(time, function(x) if ("elapsed" %in% names(x)) x["elapsed"] else NA)
  ) %>%
    ...
    mutate(Predicted_Matrix = ifelse(Method == "JRF" & Predicted_Matrix == "late", 
                                   "joint", 
                                   Predicted_Matrix))
