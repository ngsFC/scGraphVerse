...
adjm <- adjm[colnames(count_matrices[[1]]),colnames(count_matrices[[1]])]


genes <- as.data.frame(count_matrices[[1]]) %>% t() %>% as.data.frame() %>% rownames_to_column("gene") %>% filter(gene %in% rownames(adjm))

adjm <- adjm[genes$gene,genes$gene]
