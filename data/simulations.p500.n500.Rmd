...
plotROC(consensus_matrices, adjm, plot_title = "ROC voting lambda", is_binary = T)
```
### Plot comparison
...
scores.late$Statistics %>%
    datatable(extensions = 'Buttons',
              options = list(
                ...
                pageLength = 10), 
              caption = "scores")
keepscores <- scores.late$Statistics %>% mutate(Predicted_Matrix=c("vote", "union"), Method=c("ZILGM"), Ratio=nrow(adjm)/ncol(count_matrices[[1]]), p=nrow(adjm))
```
...
time[["ZILGM_early_15Cores"]] <- system.time(
  late <- infer_networks(count_matrices_list = early_matrix, 
                         method = "ZILGM", 
                         adjm = adjm, 
                         nCores = 15)
)
...
write.table(df2, file.path(ddir, "met_n500_p500.txt"), quote = FALSE, col.names = TRUE, row.names = FALSE, sep = "\t")
