...
For demonstration purposes, we'll use the built-in example data. This example dataset contains a small ground-truth network with 35 genes and their known regulatory relationships. We'll visualize this network to understand the structure we're trying to recover through our inference methods.
...
Now we'll generate synthetic single-cell count data that respects our ground-truth network structure. We simulate three experimental conditions (batches) with 50 cells each, using different parameters to model realistic batch effects and biological variability. The `zinb_simdata()` function creates zero-inflated negative binomial count data where gene expression levels are influenced by the regulatory relationships defined in our ground-truth network.
...
**GENIE3 workflow:**
1. For each target gene, build random forest model using all other genes as predictors
