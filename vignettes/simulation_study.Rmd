...
vignette: >
  %\VignetteIndexEntry{scGraphVerse Simulation Study: Simulation and  GRN Reconstruction}
...
1. Define a ground-truth regulatory network from high-confidence STRINGdb interactions.
2. Simulate zero-inflated single-cell RNA-seq count data that respects the ground truth.
3. Infer gene regulatory networks using **JRF**.
...
# Inspect sparsity
sparsity <- sapply(count_matrices, function(mat) mean(mat == 0))
```
