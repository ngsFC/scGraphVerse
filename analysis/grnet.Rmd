...
saveRDS(count_matrices, "count_matrices.RDS")
```


```{r}
add_dropout <- function(matrix, dropout_rate) {
...
for (i in 1:length(count_matrices)) {
  set.seed(1130 + i)
  count_matrices[[i]] <- add_dropout(count_matrices[[i]], dropout_rate = 0.10)
}
...
saveRDS(count_matrices, "count_matrices_drop010.RDS")
```


```{r est_sim}
#est <- PCzinb(as.matrix(t(count_matrices[[1]])), method="zinb1")
#colnames(est) <- as.character(1:10)
#graph_est <- graph_from_adjacency_matrix(est, mode="undirected")
#plot(graph_est)
#prediction_scores(adjm, est)
```
...
for (i in seq_along(count_matrices)) {
  ...
  write.csv(link_list_genie3, paste0("genie3_network_matrix_", i, ".csv"), row.names = FALSE)
  }
...
for (name in names(link_list_genie3_list)) {
  ...
  weight_cutoff <- quantile(link_list_genie3$weight, 0.70)  # Adjust this value as needed
